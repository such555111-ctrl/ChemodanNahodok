...
log = logging.getLogger("chemodan_nahodok")
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "Чемодан находок: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "🤎 *Чемодан находок*\n\n"
        "Добро пожаловать в «Чемодан находок»! Новые и б/у вещи высокого качества "
        "по доступным ценам — отправка из Москвы в любой город удобной вам службой доставки.\n\n"
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ НХ-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от «Чемодан находок» по заказу НХ-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от «Чемодан находок» по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
